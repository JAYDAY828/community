(function(){
  ...
  function requestYear(){return scope==='cumulative'?'all':String(year);}
  ...
  function mount(){
    ...
    $('w-link').onchange=()=>{const link=withdrawalState.links.find(l=>l.eventId===$('w-link').value);if(link?.type==='refund'){$('w-kind').value='refund';$('w-receipt').value=link.requestId;$('w-kind').onchange();}else if(link?.type==='distribution'){$('w-kind').value='expense';$('w-category').value='운영자 분배';$('w-kind').onchange();}};
    ...
    $('export').onclick=()=>{if(!report||!permitted())return;try{const data=C.xlsx(C.sheets(report,scope!=='monthly'?0:month,plan));const blob=new Blob([data],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=el('a');a.href=url;a.download=`정산_${scope==='cumulative'?'누적':year}${scope!=='monthly'?'':'-'+String(month).padStart(2,'0')}_KST.xlsx`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}catch(e){notice(e.message);}};
  ...
  function load(){
    ...
    const job={year:requestYear(),token:bridge.token(),generation:bridge.generation(),promise:null};
    ...
    job.promise=performLoad(job.year).finally(()=>{
      ...
      if(requestYear()!==job.year && job.token===bridge.token() && job.generation===bridge.generation())return load();
    });
    return job.promise;
  }
  async function performLoad(requestedYear){
    ...
    try{const result=await bridge.api('admin_settlement',{token,year:String(requestedYear)});
      ...
      if(requestYear()!==requestedYear)return;
      if(!result.ok)throw Error(result.error==='unknown_action'?'Apps Script에 정산 패치를 배포한 뒤 다시 조회해 주세요.':`정산 조회 실패: ${result.error}`);
      if(String(result.year)!==requestedYear)throw Error('조회 범위가 일치하지 않습니다. Apps Script를 업데이트한 뒤 다시 조회해 주세요.');
  ...
  function render(){
    ...
    const cumulative=scope==='cumulative',annual=scope!=='monthly',calendar=!annual&&mode==='calendar';
    $('year').hidden=cumulative;$('year-label').hidden=cumulative;$('month-label').hidden=annual;$('calendar-panel').hidden=!calendar;$('annual').hidden=!annual||mode==='list';
    $('modes').querySelector('[data-mode="calendar"]')?.setAttribute('aria-label',cumulative?'연도별 요약 보기':annual?'월별 요약 보기':'달력 보기');
    const calendarButton=$('modes').querySelector('[data-mode="calendar"]');if(calendarButton)calendarButton.textContent=cumulative?'연도별 요약':annual?'월별 요약':'달력';
    ...
    $('period-title').textContent=cumulative?'전체 기간 누적':annual?`${year}년`:`${year}년 ${month}월`;
    ...
    try{
      ...
      for(const [name,value,cls] of [['총 입금',t.paid,''],['취소 및 환불',t.refund,''],['순입금',t.net,'is-primary']]){
        const card=el('div',undefined,cls),valueLine=el('strong',value);valueLine.append(el('small','USDT'));
        card.append(el('span',name),valueLine);if(name==='취소 및 환불'&&t.reversal!=='0')card.append(el('small',`환불 정정 +${t.reversal}`));$('summary').append(card);
      }
      {
        $('costs').hidden=false;
        ...
        heading.append(el('strong','최종 정산'),el('small','기타 지출 · 수수료 · 본인 인출'));
        const amount=el('strong',t.afterCosts);amount.append(el('small','USDT'));
        balance.append(el('span','최종 정산액'),amount);chevron.setAttribute('aria-hidden','true');
        ...
        for(const [label,value] of [['기타 지출',t.expenses],['출금 수수료',t.fees]])dl.append(el('dt',label),el('dd',money(value)));
        adjusted.append(dl);
        ...
        owner.append(ownerHeading,ownerAmount,el('p','순입금에서 다시 차감하지 않습니다.','settlement-help'));groups.append(adjusted,owner);
        $('cost-details').append(groups,el('p','최종 정산액은 순입금에서 기타 지출·출금 수수료를 뺀 금액이며, 계좌 잔액이 아닙니다.','settlement-help'));
        const flows=period.flows;
        if(flows.length)$('cost-details').append(table(['날짜(KST)','구분','수령인','금액 USDT','수수료'],flows.map(e=>[C.kst(e.occurredAt).slice(0,16),C.eventLabel(e),e.username,C.amount(-C.units(e.amount)),e.feeAmount||'0'])));
      }
      if(calendar){
      ...
      if(annual&&mode!=='list'){
        const keys=cumulative?prepared.years:Array.from({length:12},(_,i)=>i+1);
        const rows=keys.map(k=>{const sum=cumulative?prepared.year(k).total:prepared.period(k).total;return [`${k}${cumulative?'년':'월'}`,sum.paid,sum.refund,sum.net,String(sum.count)];});
        const grid=table([cumulative?'연도':'월','총 입금 USDT','취소 및 환불 USDT','순입금 USDT','결제'],rows);grid.classList.add('settlement-annual-table');
        [...grid.querySelectorAll('tbody tr')].forEach((tr,i)=>{const cell=tr.firstElementChild,k=keys[i],button=el('button',`${k}${cumulative?'년':'월'}`);button.type='button';button.onclick=()=>{if(busy)return;scope=cumulative?'annual':'monthly';if(cumulative){year=Number(k);$('year').value=year;}else{month=k;$('month').value=month;}mode='list';day='';page=1;$('scope').value=scope;closeEditor();if(cumulative){report=null;prepared=null;render();load();}else render();host.closest('.member-dialog-surface')?.scrollTo({top:0});};cell.replaceChildren(button);});
        $('annual').append(grid,el('p',cumulative?'연도를 선택하면 해당 연도의 거래 목록을 확인할 수 있습니다.':'월을 선택하면 해당 월의 거래 목록을 확인할 수 있습니다.','settlement-help'));
      }
      $('breakdown').append(table(['플랜','입금 USDT','순입금 USDT','결제'],period.plans.length?period.plans.map(p=>[plan(p.plan),p.total.paid,p.total.net,String(p.total.count)]):[['해당 기간 내역 없음','—','—','0']]));
  ...
  function renderList(events,calendar,annual){
      ...
      if(calendar&&!day){$('list').append(el('div','날짜를 선택해 주세요','settlement-day-prompt'),el('p','선택한 날짜의 정확한 금액과 거래 정보를 이곳에서 확인할 수 있습니다.','settlement-help'));}
        ...
        if(calendar){const sum=prepared.day(day).total;$('list').append(el('p',`순입금 ${money(sum.net)}${sum.expenses!=='0'||sum.fees!=='0'?` · 최종 정산액 ${money(sum.afterCosts)}`:''}`,'settlement-day-total'));}
  ...
  function renderTransactions(shown,calendar){
    ...
    for(const r of shown.slice((page-1)*10,page*10)){
      ...
      summary.append(date,who,el('span',C.eventLabel(r),`settlement-event settlement-event--${r.eventType}`),el('strong',r.amount,'settlement-number'),el('span','보기','settlement-detail-label'));
  ...
  function recordCard(r,exception=false){const card=el('article',undefined,'settlement-record');card.append(el('strong',r.kind==='transfer'?r.username:`${r.username} · ${plan(r.plan)}`));
    card.append(el('p',`${r.kind==='transfer'?'':C.period(r.months,r.notes)+' · '}${exception?(r.kind==='receipt'?'입금일 미확인':C.labels[r.kind]):C.eventLabel(r)} · ${r.amount===null?'신청 '+money(r.requestedAmount??'미확인'):money(r.amount)}`,'settlement-amount'));
